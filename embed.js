...
var open_booking = open_booking || {};

// Set width of modal 
open_booking.width = 640;
// Set height of modal
open_booking.height = 640;
// Set name of class to trigger model
open_booking.click_class = 'oe_button';
// Set the css styles here... It would be appended to head
open_booking.css = '.oe_button{} .open-booking_noscroll{overflow: hidden;} .open_booking_iframe iframe, .open_booking_iframe{ -webkit-border-radius: 10px; -o-border-radius: 10px; -moz-border-radius: 10px; border-radius: 10px;} .open_booking_iframe iframe{ overflow: hidden;}';

open_booking.over = open_booking.ifrm = {};
open_booking.init = function (os_ns) {
    ...
    bind_event(document, 'click', function (e) {
        ...
        if (typeof idd != 'undefined' && idd != null) {
            var paras = getElementsByClassName(document, 'open_booking_iframe');
            while (paras[0]) {
                paras[0].parentNode.removeChild(paras[0]);
            }
            document.body.className = document.body.className.replace(/\bopen-booking_noscroll\b/, '');
            if (document.getElementById("open_booking_iframe_mask")) document.getElementById("open_booking_iframe_mask").remove();
        }
    });
    ...
    for (i = 0; i < os_ns._eles.length; i++) {
        ...
        bind_event(os_ns._eles[i], 'click', function (e) {
            bod = document.body;
            bod.className += " open-booking_noscroll";
            ...
            over.setAttribute("id", 'open_booking_iframe_mask');
            over.style.top = 0;
            over.style.left = 0;
            over.style.margin = 0;
            ...
            ifrmc.setAttribute("id", 'open_booking_iframe' + i);
            ifrmc.setAttribute("class", 'open_booking_iframe');
...
window.onload = function () {
    open_booking.init(open_booking);
};